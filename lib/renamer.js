...
class Renamer {
    ...
    getProgram() {
        ...
        return syobocalRss2.request({
            ...
            if (filter.length === 0) {
                ...
                // 見つからなかった場合
                let input = path.basename(this.options.input);
                let ext = path.extname(this.options.input);
                input = input.replace(ext, '');

                console.log('inputFilename: '+input);

                let handbrakeCommand = 'ts HandBrakeCLI -i "'+this.options.input+'" -t 1 -o "'+this.options.parent+input+'.mp4" -f mp4 --detelecine --decomb ';
                if(input.indexOf('1080p') != -1) {
                    handbrakeCommand += '-w 1920 -l 1080 -q 19';
                } else if(input.indexOf('720plow') != -1){
                    handbrakeCommand += '-w 1280 -l 720 -q 24';
                } else {
                    handbrakeCommand += '-w 1280 -l 720 -q 21';
                }
                handbrakeCommand += ' -e x264 --pfr -a 1 -E fdk_aac -6 stereo -R Auto -B 192 -D 0 --gain 0 --crop 0:0:0:0 -x merange=32:deblock=-2,-2:b-adapt=2:rc-lookahead=50:ref=6:bframes=6:me=umh:analyse=all:trellis=2:no-fast-pskip=1:scenecut=70:b-pyramid=strict --x264-preset=fast --verbose=1';
                handbrakeCommand += " && ts " + "chmod -R 775 '"+this.options.parent+"'";

                const execSync = require('child_process').execSync;
                execSync(handbrakeCommand).toString();
    ...
    rename() {
        ...
        return new Promise((resolve, reject) => {
            ...
            // console.log('inputPath: ' + this.options.input);
            // console.log('outputPath: ' + this.output.path);
            // console.log(this.output);

            let handbrakeCommand = 'ts HandBrakeCLI -i "'+this.options.input+'" -t 1 -o "'+this.options.parent+'/'+this.output.dir+'/'+this.output.file+'.mp4" -f mp4 --detelecine --decomb ';
            let inputFilename = this.options.input;
            if(inputFilename.indexOf('1080p') != -1) {
                handbrakeCommand += '-w 1920 -l 1080 -q 19';
            } else if(inputFilename.indexOf('720plow') != -1){
                handbrakeCommand += '-w 1280 -l 720 -q 24';
            } else {
                handbrakeCommand += '-w 1280 -l 720 -q 21';
            }
            handbrakeCommand += ' -e x264 --pfr -a 1 -E fdk_aac -6 stereo -R Auto -B 192 -D 0 --gain 0 --crop 0:0:0:0 -x merange=32:deblock=-2,-2:b-adapt=2:rc-lookahead=50:ref=6:bframes=6:me=umh:analyse=all:trellis=2:no-fast-pskip=1:scenecut=70:b-pyramid=strict --x264-preset=fast --verbose=1';
            handbrakeCommand += " && ts " + "chmod -R 775 '"+this.options.parent+"'";
            handbrakeCommand += " && ts " + "chmod -R 775 '"+this.options.parent+this.output.dir+"'";
